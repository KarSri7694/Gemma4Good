...
class DatabaseManager:
    ...
    def _run_migrations(self, connection: sqlite3.Connection) -> None:
        self._ensure_column(connection, "assessment_questions", "options_json", "TEXT")
        self._ensure_column(connection, "students", "email", "TEXT")
        self._ensure_column(connection, "student_answers", "grading_reasoning", "TEXT")
        self._ensure_column(connection, "student_blueprints", "subject", "TEXT NOT NULL DEFAULT ''")
        ...
        connection.execute(
            "CREATE INDEX IF NOT EXISTS idx_academic_year_plans_class_subject "
            "ON academic_year_plans (class_id, subject, academic_year)"
        )
        connection.execute(
            "CREATE INDEX IF NOT EXISTS idx_academic_year_plan_units_plan_order "
            "ON academic_year_plan_units (plan_id, sequence_order)"
        )
        connection.execute(
            "CREATE INDEX IF NOT EXISTS idx_class_timetable_slots_class_subject_day "
            "ON class_timetable_slots (class_id, subject, weekday)"
        )
        connection.execute(
            "CREATE INDEX IF NOT EXISTS idx_class_coverage_sessions_class_subject_date "
            "ON class_coverage_sessions (class_id, subject, session_date)"
        )
        connection.execute(
            """
        ...
        self._cleanup_decimal_subtopic_chapters(connection)
    ...
    def _cleanup_decimal_subtopic_chapters(self, connection: sqlite3.Connection) -> None:
        ...
        for row in rows:
            ...
            if is_decimal_code or is_decimal_name:
                ...
                legacy_rows_to_delete.append((str(board_type), str(grade), str(subject), code))
        if ids_to_delete:
...
DEFAULT_DATABASE = DatabaseManager()


def get_connection() -> sqlite3.Connection:
    return DEFAULT_DATABASE.get_connection()
