...
from app.gemma_adaptation_profile import DEFAULT_ADAPTATION_PROFILE_GENERATOR
from app.gemma_blueprint import DEFAULT_BLUEPRINT_GENERATOR
from app.gemma_grader import DEFAULT_GEMMA_GRADER
from app.google_forms import GoogleFormsClient
...
class AssessmentSyncService:
    def __init__(
        ...
        *,
        ...
        grader=DEFAULT_GEMMA_GRADER,
        blueprint_generator=DEFAULT_BLUEPRINT_GENERATOR,
        adaptation_generator=DEFAULT_ADAPTATION_PROFILE_GENERATOR,
    ) -> None:
        self.forms_client = forms_client or GoogleFormsClient()
        self.grader = grader
        self.blueprint_generator = blueprint_generator
        self.adaptation_generator = adaptation_generator
        self.sampling = load_model_sampling_config()
    ...
    def sync_google_form_assessment(
        ...
        *,
        ...
        responses_seen = self.enqueue_new_google_form_responses(assessment_id=assessment_id)
        processed = 0
        while True:
            result = self.process_next_queued_response(
        ...
        bundle = assessment_repository.get_assessment_sync_bundle(assessment_id)
        student_mastery_rows, class_mastery_rows = self.recompute_assessment_mastery(bundle)
        analytics_repository.replace_mastery_snapshots(
    ...
    def enqueue_new_google_form_responses(self, *, assessment_id: int | None = None) -> int:
        assessments = (
            [assessment_repository.get_assessment_sync_bundle(assessment_id)["assessment"]]
            if assessment_id
            else assessment_repository.list_google_linked_assessments()
        )
        enqueued_count = 0
        for assessment in assessments:
            if not assessment or not assessment.get("google_form_id"):
                continue
            responses = self.forms_client.list_google_form_responses(form_id=assessment["google_form_id"])
            for response in responses:
                created = queue_repository.enqueue_response_for_processing(
    ...
    def process_next_queued_response(
        ...
        *,
        ...
        queue_item = queue_repository.claim_next_queued_response()
        if not queue_item:
            return None

        if assessment_id_filter is not None and queue_item["assessment_id"] != assessment_id_filter:
            queue_repository.mark_queue_item_failed(
                queue_item["id"],
                "Queue item belonged to a different assessment filter.",
            )
            return None

        try:
            response = json.loads(queue_item["raw_response_json"])
            bundle = assessment_repository.get_assessment_sync_bundle(queue_item["assessment_id"])
            if not bundle:
            ...
            answers_payload = self._extract_answer_rows(
                response=response,
            ...
            assessment_repository.upsert_google_form_response_sync(
            ...
            self._refresh_student_blueprint(
                student_id=student["id"],
                class_id=bundle["assessment"]["class_id"],
            ...
            self._refresh_student_adaptation_profile(
                student_id=student["id"],
                class_id=bundle["assessment"]["class_id"],
                subject=bundle["assessment"]["subject"],
                last_submission_at=response.get("lastSubmittedTime"),
            ...
            queue_repository.mark_queue_item_completed(queue_item["id"])
            return {"assessment_id": queue_item["assessment_id"], "student_id": student["id"]}
        except Exception as exc:
            queue_repository.mark_queue_item_failed(queue_item["id"], str(exc))
            return None

    def recompute_assessment_mastery(
        self,
        bundle: dict[str, Any],
    ) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
        with DEFAULT_DATABASE.get_connection() as connection:
        ...
        return self._aggregate_mastery_rows(mastery_input_rows=mastery_input_rows)
    ...
    def _extract_answer_rows(
        ...
        *,
        ...
        for google_question_id, answer in answers.items():
            ...
            raw_answer = self._extract_text_answer(answer)
            grading = self._grade_response(
                question=question,
    ...
    def _extract_text_answer(self, answer: dict[str, Any]) -> str:
    ...
    def _fallback_correctness(self, question: dict[str, Any], raw_answer: str) -> bool:
    ...
    def _grade_response(
        ...
        *,
        ...
        if question["question_type"] == "mcq":
            is_correct = self._fallback_correctness(question, raw_answer)
            return {
        ...
        graded = self.grader.grade_answer_with_gemma(
            client=llama_client,
    ...
    def _refresh_student_blueprint(
        ...
        *,
        ...
        context = student_repository.get_student_blueprint_context(student_id, subject)
        assessment_count = len(context.get("assessment_history", []))
        blueprint = self.blueprint_generator.generate_student_blueprint_with_gemma(
            client=llama_client,
            model_name=llama_model_name,
            temperature=self.sampling.temperature,
            top_p=self.sampling.top_p,
        ...
        student_repository.upsert_student_blueprint(
    ...
    def _refresh_student_adaptation_profile(
        ...
        *,
        ...
        assessment_count = len(context.get("assessment_history", []))
        generated = self.adaptation_generator.generate_student_adaptation_profile_with_gemma(
            client=llama_client,
            model_name=llama_model_name,
            temperature=self.sampling.temperature,
            top_p=self.sampling.top_p,
...
DEFAULT_ASSESSMENT_SYNC_SERVICE = AssessmentSyncService()
...
def sync_google_form_assessment(
    ...
    *,
    ...
    return DEFAULT_ASSESSMENT_SYNC_SERVICE.sync_google_form_assessment(
        assessment_id,
        llama_base_url=llama_base_url,
        llama_model_name=llama_model_name,
    )
...
def enqueue_new_google_form_responses(*, assessment_id: int | None = None) -> int:
    return DEFAULT_ASSESSMENT_SYNC_SERVICE.enqueue_new_google_form_responses(assessment_id=assessment_id)
...
def process_next_queued_response(
    *,
    ...
    return DEFAULT_ASSESSMENT_SYNC_SERVICE.process_next_queued_response(
        llama_base_url=llama_base_url,
        llama_model_name=llama_model_name,
        assessment_id_filter=assessment_id_filter,
    )


def recompute_assessment_mastery(bundle: dict[str, Any]) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    return DEFAULT_ASSESSMENT_SYNC_SERVICE.recompute_assessment_mastery(bundle)
