...
def grade_answer_with_gemma(
    *,
    ...
    response = client.chat_completion(
        messages=_build_grading_messages(question=question, student_answer=student_answer),
    ...
    parsed = _parse_grading_response(response)
    if parsed:
        return parsed
    return _fallback_grade(question=question, student_answer=student_answer)


def _build_grading_messages(*, question: dict[str, Any], student_answer: str) -> list[dict[str, str]]:
    schema_hint = {
        "score_awarded": 2,
        "is_correct": True,
        "feedback": "string",
...
def _parse_grading_response(response: dict[str, Any]) -> dict[str, Any] | None:
    content = ""
...
def _fallback_grade(*, question: dict[str, Any], student_answer: str) -> dict[str, Any]:
